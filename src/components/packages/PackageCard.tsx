...
export default function PackageCard({ pkg: packageObj }: { pkg: PackageDTO }) {
  // const { packages, toursByPackage } = usePackageStore(); // Accessing package and tour data from the store
  const toursByPackage = usePackageStore((s) => s.toursByPackage);

  // Tours for THIS package only
  const pkgTours =
    typeof packageObj.id === "string"
      ? (toursByPackage[packageObj.id] ?? [])
      : []; // id missing on first render -> fall back to [] // ✅ keyed access

  const image0 = packageObj.images?.[0] ?? "/assets/images/destination/d-1.png";
  const tourCount = pkgTours.length; // or packageObj.tours?.length if you store ids on the pkg
        ...
        <div className="row destinations-1">
          ...
          >
            <div className="swipper-wrapper">
              ...
              {pkgTours.map((tour) => (
                <SwiperSlide key={tour.id ?? tour.name}>
